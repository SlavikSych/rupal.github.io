...
$_SESSION["EMAIL"]=$email;
  ...
  if (!preg_match("/^[а-я А-Я a-z A-Z ]{2}+[а-яА-Яa-zA-Z\-\ ]+$/u",$name)) {
      $nameErr = "Данные введены некорректно!"; 
       $error=true;
    }
  }
...
if (empty($_POST["cityFF"])) {
    ...
    // check if name only contains letters and whitespace
  if (!preg_match("/^[а-яА-Яa-zA-Z ]{2}+[а-яА-Яa-zA-Z ]+$/u",$city)) {
      $cityErr = "Данные введены некорректно!"; 
       $error=true;
    }
}
  ...
  if (empty($_POST["messageFF"])) {
    ...
    $tel = test_input($_POST["messageFF"]);
     ...
     // $pattern1='/((8|\+7)-?)? \(?\d{3,5}\)?-? \d{1}-?\d{1}-?\d{1}-?\d{1}-?\d{1}((-?\d{1})?-?\d{1})?/'; з робілами
     $pattern1='/((8|\+7)-?)?\(?\d{3,5}\)?-?\d{1}-?\d{1}-?\d{1}-?\d{1}-?\d{1}((-?\d{1})?-?\d{1})?/';//без пробілів
		...
		<section class="slider" id="menu-slider">
			<div class="container">
				<div class="row">
					...
					<div class="col-sm-6">
						<form enctype="multipart/form-data" method="post" id="feedback-form" accept-charset="utf-8" class="row suscribe">
						<div class="col-sm-12">
